...
def run_server():
    ...
    subprocess.run(
        [sys.executable, "-m", "uvicorn", "omnimem.api:app", "--host", "0.0.0.0", "--port", "8000"], check=True
    )
...
def run_worker():
    ...
    subprocess.run(
        [sys.executable, "-m", "celery", "-A", "omnimem.celery_app", "worker", "--loglevel=info"], check=True
    )

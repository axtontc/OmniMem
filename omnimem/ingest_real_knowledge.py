...
async def main():
    ...
    memory_create = SemanticMemoryCreate(
        concept_name=concept, text_content=content, embedding=vector, metadata={"source": "Axton", "confidence": 1.0}
    )
    ...
    bus_entity = Entity(
        id="concept_redis", label="Concept", properties={"name": "Redis Event Bus", "type": "Infrastructure"}
    )
    relation = Relation(
        source_id="concept_swarm", target_id="concept_redis", type="USES_INFRASTRUCTURE", properties={"latency": "<5ms"}
    )
    ...
    print("[+] Stored Topological Graph in Neo4j:")
    print("    (Fractal Swarm) -[USES_INFRASTRUCTURE]-> (Redis Event Bus)")
    ...
    if results:
        print("[RETRIEVED VECTOR] Distance match found!")
    ...
    with neo4j_driver.session() as session:
        ...
        if record:
            print("[RETRIEVED GRAPH] Subgraph match found!")

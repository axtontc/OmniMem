...
def _ingest_vector_logic(payload: dict):
    ...
    async def do_write():
        ...
        try:
            await db.store_semantic_memory(SemanticMemoryCreate(
...
def ingest_vector(self, **kwargs):
    try:
        return wal.execute_with_lock(
            task_name="ingest_vector",
            payload=kwargs,
            operation=lambda: _ingest_vector_logic(kwargs)
        )
    except Exception as exc:
        raise self.retry(exc=exc, countdown=2 ** self.request.retries)
...
def _ingest_graph_logic(payload: dict):
    ...
    asyncio.run(do_write())
    return {"status": "success", "entities_count": len(payload.get('entities', [])), "relationships_count": len(payload.get('relationships', []))}

@app.task(bind=True, max_retries=3, name='tasks.ingest_graph')
def ingest_graph(self, **kwargs):
    try:
        return wal.execute_with_lock(
            task_name="ingest_graph",
            payload=kwargs,
            operation=lambda: _ingest_graph_logic(kwargs)
        )
    except Exception as exc:
        raise self.retry(exc=exc, countdown=2 ** self.request.retries)

...
def test_db_search_script():
    async def test():
        ...
        res = await pool.fetch("SELECT concept_name, embedding <=> $1::vector as dist FROM semantic_memory WHERE embedding <=> $1::vector < 0.95 ORDER BY embedding <=> $1::vector LIMIT 10", prompt_emb_str)
        for r in res:

...
from omnimem.security_contract import AgentSwarmMessage, ContractViolationError, MemMCPHookPayload
...
async def test_pub_sub_latency():
    ...
    payload = MemMCPHookPayload(
        version="1.0", event_type="test_event", content="This is a fast test event", metadata={"source": "pytest"}
    )
...
async def test_schema_validation_and_subscription():
    ...
    valid_payload = AgentSwarmMessage(
        version="1.0", agent_id="agent_123", action="update", target="knowledge_base", payload={"data": "test"}
    )
    ...
    async def mock_listen():
        yield {
            "type": "message",
            "data": '{"version": "1.0", "agent_id": "agent_123", "action": "update", "target": "knowledge_base", "payload": {"data": "test"}}',
        }
    ...
    async def mock_bad_listen():
        yield {
            "type": "message",
            "data": '{"version": "1.0", "agent_id": "bad", "action": "DROP TABLE", "target": "db", "payload": {}}',
        }

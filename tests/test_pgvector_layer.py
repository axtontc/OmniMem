...
async def test_database_integrity_error_propagation(mock_pool):
    ...
    data = EpisodicLogCreate(
        agent_id="agent_1", event_type="thought", event_content="Thinking...", embedding=[0.2] * 384
    )
...
async def test_contract_violation_on_invalid_data():
    with pytest.raises(ContractViolationError):
        SemanticMemoryCreate(
            ...
            text_content="This is a test concept",
            embedding=[0.1]
            * 10,  # Validates purely by typing at the moment, assuming List[float] is checked by Pydantic.
            metadata={"source": "test"},
        )
